...
def metrics(y_true, y_pred):
    ...
    for i in range(len(y_true)):
        if y_true[i] != 0:
            y_true_posneg.append(y_true[i])
            y_pred_posneg.append(y_pred[i])

    f1_posneg_macro = round(f1_score(y_true_posneg, y_pred_posneg, average='macro') * 100, 2)
    f1_posneg_micro = round(f1_score(y_true_posneg, y_pred_posneg, average='micro') * 100, 2)
